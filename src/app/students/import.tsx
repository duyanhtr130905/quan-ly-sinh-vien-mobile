...
export default function StudentImportScreen() {
  ...
  const previewMutation = useMutation({ mutationFn: () => previewStudentImport(file!.uri, file!.name), onSuccess: (response) => { setPreview(response.data); setDrafts(response.data.rows); setValidatedFingerprint(importFingerprint(response.data.rows)); } });
  const validateMutation = useMutation({ mutationFn: (items: StudentImportRow[]) => validateStudentImport(items), onSuccess: (response) => { setPreview(response.data); setDrafts(response.data.rows); setValidatedFingerprint(importFingerprint(response.data.rows)); } });
  ...
  const hasCurrentValidation = validatedFingerprint === importFingerprint(drafts);
  ...
  const pickFile = async () => {
    ...
    setFile({ uri: asset.uri, name: asset.name, size: asset.size ?? null }); setPreview(null); setDrafts([]); setValidatedFingerprint(null);
  };
  const updateRow = (draftKey: string, values: StudentImportValues) => { setDrafts((current) => current.map((row) => row.draftKey === draftKey ? { ...row, values } : row)); setValidatedFingerprint(null); };
